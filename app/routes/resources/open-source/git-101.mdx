...
If you have a repositroy on Github or another code sharing platform or on a server somewhere, you clone it. To do this, you can run `git clone <Repo>` where the Repo is the URL of the repository. This will set up a copy of the repository on your local computer, with the remote repository you cloned from as the `origin` remote. Remember cloning is different from forking, for an explanation of forking, see the Github section below.

#### Adding another remote

When working on open source, you will normally clone a forked version of the repository, but you may want to add the original version as a remote as well so you can pull other people's changes into your local repository. This remote is conventionally called `upstream` and can be added using `git remote add upstream <Repo>` where Repo is the URL of the original repository.
...
The following workflows are useful when collaborating with another person in Open Source

#### Workflow for checking out someone elses branch on your local machine

Let's assume that you and your teammate, Alice, forked a project repository and you want to go to Alice's branch in your local machine.

1. You add Alice's fork as a remote on your local repository using `git remote add alice <Alice's Repo URL>`.
2. You fetch Alice's branches using `git fetch alice`.
3. You can now go to Alice's branch using `git checkout alices-branch-name`.
4. You can now pull changes pushed by Alice using `git pull alice alices-branch-name`.

#### Workflow for collaborating on a shared branch between two different forks

Let's assume that you and another teemmate, Bob, both have a fork of a project and want to collaborate on a contribution using one branch:

1. You adds Bob's fork as a remote on your local repository using `git remote add bob <Bob's Repo URL>`.
2. Bob adds your fork as a remote on his local repository using `git remote add your-name <Alice's Repo URL>`.
3. You can now pull changes pushed by Bob using `git pull bob branch-name`.
4. Bob can now pull changes pushed by You using `git pull your-name branch-name`.
5. Either you or Bob can submit the pull request once they have all the work on their fork of the project.
6. Each commit will automatically contain either your or Bob's name, so each person's contributions can be identified.

`git fetch` can be used to fetch branches from someone elses remote, for example Bob can run `git fetch your-name` in the above scenario to fetch all your branches.
...
What is Github? Github is a platform for hosting, sharing and collaborating on Git repositories. Gitlab and Bitbucket are two other similar platforms.
...
This is an example of one of our member's profiles built using a profile readme:
