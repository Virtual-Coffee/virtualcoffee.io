...
3. Choose how you want to clone it — through HTTPS, SSH, or GitHub CLI. <br /> For this example, we will use HTTPS.
4. Copy the URL.
...
Now you're ready to work on the changes. Remember to always work on changes in a new branch, not directly in the `main` or `master` branch.
...
3. If the buttons are inactive, the repository has no changes. You can <Link to="/resources/developer-resources/open-source/git-101#push-changes">push your changes</Link> and <Link to="/resources/developer-resources/open-source/git-101#making-a-pull-request">make a pull request</Link> in that case. <br />
...
Now that your `origin` repository is in sync with the `upstream`, you also want to synchronize these changes on your local. That way, you already have the most updated content when you push your changes.
...
In this step, you might need to resolve some conflicts, if any.
...
2. In the local environment
   ...
   - Do all the steps in <Link to="/resources/developer-resources/open-source/git-101#fetching-upstream-changes">Fetching `upstream` changes</Link> and <Link to="/resources/developer-resources/open-source/git-101#synchronizing-changes-locally">Synchronizing changes locally</Link>.<br />  
...
After resolving the conflicts, if necessary, you can fix and adjust the codes. Then <Link to="/resources/developer-resources/open-source/git-101#add-and-commit-changes">add and commit the changes</Link>.
...
You or Alice can submit the pull request once you both have done all the work.
Each commit will automatically contain either your or Alice's name. So, both of your contributions are identified.

### Synchronous collaboration

Say sometimes you work synchronously with Alice on the same issue. All steps are the same as the asynchronous collaboration above. But in sync collaboration, you want to [include them in your commit message](https://docs.github.com/en/pull-requests/committing-changes-to-your-project/creating-and-editing-commits/creating-a-commit-with-multiple-authors), so you can commit together.
...
- [Mini Portfolio: Bring Your GitHub Profile To The Next Level — Ayu Adiati](https://adiati.com/mini-portfolio-bring-your-github-profile-to-the-next-level)
