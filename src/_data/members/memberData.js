...
async function getMemberGithubData(data) {
	...
	let asset = new AssetCache('memberGithubData');
	...
	try {
		...
		await asset.save(githubData, 'json');
...
module.exports = async function () {
	...
	const memberData = {
		members: fs
			...
			.filter((filename) => filename !== '_EXAMPLE.js')
			.map((filename) => require(path.resolve(__dirname, 'members', filename))),
		core: fs
	...
	const githubData = await getMemberGithubData([
		...memberData.core,
		...memberData.members,
	]);
