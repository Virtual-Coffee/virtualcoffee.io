...
- `/invites` has its own sign-in page, so a Volunteer is never sent to a
  page titled "Maintainer sign-in". The two share the button, provider and
  callback; `/admin/sign-in` also redirects a Volunteer to `/invites` rather
  than telling them they are "not an admin".
