...
${
	...
	eleventyConfig.addPairedShortcode('leadText', function (content) {
		return `<div class="lead">${content}</div>`;
	});

	eleventyConfig.addShortcode('backToTopLink', function () {
		return `<p class="text-right"><a href="#top">Back to Top</a></p>`;
	});
	...
	eleventyConfig.addShortcode(
		...
		function ({ labelText, helpText, ...inputAttributes }) {
			const { processedHelpText, helpTextId, attributes, inputId } =
	...
	eleventyConfig.addShortcode(
        ...
        <select ${attributes.join(' ')}>
          ${options
						.map((opt) => {
							if (opt.optgroup) {
								return `<optgroup label="${opt.optgroup}">${opt.options
