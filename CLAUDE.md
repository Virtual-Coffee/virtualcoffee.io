...
Events are read from the public Google Events Calendar, which is the system of record: the Join Link is the event's `location`, `extendedProperties` are never read or written, and nothing on the calendar is private. `/admin/events` edits it through the same service account — every write carries the `etag` it loaded as `If-Match`, a Series is Ended (`UNTIL` now) rather than deleted, and editing a Series applies to every Event of it. `docs/adr/0014`.
